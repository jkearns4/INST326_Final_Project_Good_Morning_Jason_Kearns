    ...
    def generate_entities(self):
        ...
        for trap in trap_locations:
            if random.random()>=0.5:
                clue_messages.append(f"Careful, there is a trap on row {trap[0]+1}")
            else:
                clue_messages.append(f"Careful, there is a trap on column {trap[1]+1}")
        
        
        clue_locations = []
        
        for i in range(len(clue_messages)):
            clue_location = random.choice(available_locations)
            available_locations.remove(clue_location)

            clue = e.Clue(f"Clue {i + 1}", clue_messages.pop(0))
            self.place_clue(clue_location[0], clue_location[1], clue)
            
            clue_locations.append((clue_location[0], clue_location[1]))
